...
function mockSuccessResponse(statusCode, body, capture) {
    ...
    return (options, callback) => {
        ...
        const mockRes = { statusCode, on(event, handler) { if (event === "data")
                handler(bodyStr); if (event === "end")
                handler(); } };
        const mockReq = { _errorHandler: null, on(event, handler) { if (event === "error")
                mockReq._errorHandler = handler; return mockReq; }, write(data) { if (capture)
                capture.written = String(data); }, end() { callback(mockRes); } };
        return mockReq;
    };
}
function mockNetworkError(message) {
    return (_options, _callback) => {
        const mockReq = { _errorHandler: null, on(event, handler) { if (event === "error")
                mockReq._errorHandler = handler; return mockReq; }, write() { }, end() { setImmediate(() => { if (mockReq._errorHandler)
