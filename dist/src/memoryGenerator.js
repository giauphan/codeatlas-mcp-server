...
export function generateMemory(workspaceRoot, analysis) {
    ...
    const stats = {
        ...
        dependencies: links.filter((l) => l.type === 'import').length,
    ...
    const keyModulesSection = keyModules
        .map((m, i) => `${i + 1}. **${m.name}** (${m.connections} connections) — \`${path.relative(workspaceRoot, m.path || '')}\``)
        .join('\n');
    ...
    const patterns = [];
    if (modules.some((m) => m.filePath?.includes('controller') || m.filePath?.includes('Controller'))) {
        patterns.push('- ✅ MVC Pattern (Controllers detected)');
    }
    if (modules.some((m) => m.filePath?.includes('service') || m.filePath?.includes('Service'))) {
        patterns.push('- ✅ Service Layer (Services detected)');
    }
    if (modules.some((m) => m.filePath?.includes('model') || m.filePath?.includes('Model'))) {
        patterns.push('- ✅ Model Layer (Models detected)');
    }
    if (modules.some((m) => m.filePath?.includes('middleware') || m.filePath?.includes('Middleware'))) {
        patterns.push('- ✅ Middleware Pattern');
    }
    if (modules.some((m) => m.filePath?.includes('test') || m.filePath?.includes('spec'))) {
