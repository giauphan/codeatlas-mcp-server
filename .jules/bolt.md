...
## 2024-05-25 - [Performance improvement] Optimized O(N) array filtering and mapping for node suggestions
**Learning:** Found an O(N) array iteration and allocation bottleneck in `mcpServer.ts` (around line 1084) where an array of nodes was subjected to chained `.filter(n => n.type === "module" && n.filePath).map(n => n.label).slice(0, 10)`. Although this generates 10 suggestions, `.filter()` forces a traversal of the entire graph and allocates multiple intermediate arrays.
**Action:** When extracting a small slice of processed data from a large array, avoid chained `.filter().map().slice()`. Instead, use a `for...of` loop with a manual array `.push()` and an early `break` condition (`if (suggestions.length >= 10) break`) to stop iteration immediately once the desired number of elements is found, saving O(N) traversal time and memory allocation.
