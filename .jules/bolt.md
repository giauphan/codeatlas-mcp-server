...
## 2026-08-05 - [Performance improvement] Optimized O(N) single-pass mapping
**Learning:** Chained array methods like `.filter().map()` applied multiple times sequentially on the same large array to extract different entity types (e.g., extracting modules, classes, and functions from a nodes array) forces multiple full traversals and generates numerous intermediate array allocations. This causes high CPU usage and significant GC overhead.
**Action:** When categorizing a large collection into several subsets and transforming them, use a single `for...of` loop. Evaluate the conditions and `push` the transformed items directly into their respective pre-allocated arrays, accomplishing the same task in a single O(N) pass.
