...
## 2024-07-01 - [Rejected] Premature optimization of .filter().slice().map() on small arrays
**Learning:** I attempted to optimize a `.filter().slice().map()` chain in `src/securityScanner.ts` by replacing it with a `for...of` loop to prevent O(N) traversals and intermediate allocations. However, the array being processed (`findings`) was very small, and the code path was not a hot loop. The change sacrificed readability for a negligible micro-optimization, violating Bolt's core philosophy of avoiding premature optimization without measurable impact.
**Action:** Do not replace chained array methods like `.filter().slice().map()` with `for...of` loops unless the data set is demonstrably large (e.g., thousands of items) or the code is executed in a highly frequent hot path. Prioritize readability over micro-optimizations.
