...
## 2024-09-19 - [Performance improvement] Optimized O(L) link filtering with early exit
**Learning:** When retrieving a limited number of connections (e.g. 50 relationships) for a subgraph, using `links.filter(...).slice(0, 50).map(...)` over the entire graph's `links` array forces a full O(L) traversal and creates intermediate array allocations. For large codebases with hundreds of thousands of links, this adds significant unnecessary CPU and GC overhead.
**Action:** Replace the chained `.filter().slice().map()` pattern with a single `for...of` loop over the links, directly format and `push()` matching relationships, and `break` as soon as the limit is reached.
