...
## 2024-07-01 - [Performance improvement] Optimized chained array .filter().slice().map() operations on security findings
**Learning:** In the `aiScan` method of `src/securityScanner.ts`, logic utilizing chained array methods (`.filter().slice().map()`) extracted a limited set of critical security findings and stringified them. This pattern forced an O(N) traversal in the filter phase and created two intermediate array allocations before creating the final string.
**Action:** When filtering, mapping, and slicing subsets from arrays where an early exit is possible, you can refactor `.filter().slice().map()` chains by substituting them with a single `for...of` loop. Maintain a bounded array by utilizing `.push()` directly and early-exiting the loop (`if (arr.length >= limit) break;`) to avoid two intermediate array allocations and achieve an early exit when the matches appear early. Note that for small lists, this is a micro-optimization and `.filter().slice().map()` is often more readable.
