...
## 2024-05-19 - [Optimization] Use Set instead of Array for repeated O(N) array filtering
...
**Action:** Always watch for large arrays that are iteratively filtered or searched with `.indexOf` or `.includes` inside loops, especially when maintaining collections of paths or IDs. Replace them with `Set` or `Map` to prevent severe CPU and event loop bottlenecks in high-volume scenarios. Note that `Set` or `Map` should only be used when the dataset is expected to have unique keys or IDs. For cases where duplication is valid or sorting matters, arrays may still be the preferred data structure.
