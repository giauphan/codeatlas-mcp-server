...
## 2026-08-30 - [Performance improvement] Optimized O(N log N) Array Sorting in Diagram Scope
**Learning:** Using `Array.prototype.sort()` coupled with `Array.prototype.indexOf()` inside the custom comparator to re-order an array of graph nodes by categorical priorities (e.g. `['module', 'class', 'function', 'variable']`) creates an unnecessary `O(N log N)` bottleneck, especially inside chunk processing logic or diagram truncation steps.
**Action:** When ordering large arrays by a predefined set of categories, use an `O(N)` bucket-collection strategy to linearly gather elements into respective category arrays using a single pass (e.g. `const buckets = { module: [], ... }`), then concatenate them at the end. This is significantly faster and inherently stable.
