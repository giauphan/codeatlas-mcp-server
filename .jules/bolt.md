...
## 2024-05-24 - [Performance improvement] Optimized O(N^2) array.includes() in loops
**Learning:** Calling `array.includes(item)` inside a loop over another collection (e.g., when cross-referencing nodes) causes an O(N^2) time complexity bottleneck. This is because `includes()` does a linear O(N) scan for each element of the outer loop.
**Action:** When performing repeated existence checks on an array inside a loop, always convert the array to a `Set` (e.g., `const mySet = new Set(myArray);`) outside the loop and use `mySet.has(item)` for O(1) lookups.
