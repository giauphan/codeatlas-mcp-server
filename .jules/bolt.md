...
## 2024-05-24 - [Performance improvement] Optimized O(M) intermediate array allocation when slicing iterators
**Learning:** Using `Array.from(map.values()).slice(0, N)` to extract a limited subset of items from an Iterable forces a full O(M) traversal (where M is the total size of the Map/Set), allocating a massive intermediate array in memory before throwing most of it away. For large iterables in hot paths, this causes significant Garbage Collection overhead.
**Action:** When extracting at most N items from an Iterable, avoid `Array.from(iter).slice(0, N)`. Instead, use an early exit `for...of` iterator (e.g. `takeFromIterable` utility) to break out of the loop exactly when N items are collected.
