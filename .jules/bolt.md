...
## 2024-06-25 - [Performance improvement] Optimized O(N) array filtering and mapping allocations
**Learning:** During analysis of the `trace_relationships` and `code_search` components inside `mcpServer.ts`, an inefficient logic pattern was found utilizing multiple chained array methods (`.filter().map().slice()` and `.slice().map().filter()`). The first instance needlessly processed up to thousands of graph nodes when only 10 items were desired. The second instance needlessly constructed and discarded intermediate arrays to filter falsy empty strings and format lines.
...
## 2025-01-20 - [Performance improvement] Optimized O(N) array filtering allocations and respected PR review feedback
**Learning:** During review of replacing `.filter().slice().map()` with a single `for...of` loop in `src/securityScanner.ts`, some engineers might perceive it as an unnecessary micro-optimization and advocate for the declarative array methods. However, prioritizing performance optimization on this project dictates that intermediate array allocations should be minimized, particularly when traversing potentially large arrays. Also, removing "promotional noise" from code comments and ensuring magic numbers are extracted into constants are important parts of clean code and smooth code reviews.
**Action:** When replacing chained array methods with a single loop, ensure the logic uses named constants for limits, uses clean string template literals, and avoids excessive promotional comments to minimize PR pushback, while still maintaining the core optimization.
