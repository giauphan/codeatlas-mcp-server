...
## 2025-01-01 - [Performance improvement] Optimized O(N) array filtering allocations
**Learning:** When dealing with array chaining like `.filter().slice(0, N)`, using a `for...of` loop with an early `break` is a true optimization because it prevents iterating over the entire array after the desired limit is reached. However, replacing simple `.map().filter()` chains with `for` loops on small arrays (like those in `src/cli/commands.ts`) is a micro-optimization that sacrifices readability without measurable performance gains.
**Action:** Focus array chaining refactors primarily on scenarios where an early exit condition (`break`) can dramatically reduce time complexity, or where the arrays involved are proven to be massive. Avoid applying it purely to save minimal intermediate array allocations on small datasets.
