...
## 2026-08-03 - [Performance improvement] Optimized O(N) array filtering and mapping in tests/mcpServer
**Learning:** Found multiple places in `mcpServer.ts` and `e2e.test.ts` where arrays were traversed repeatedly using multiple `.filter().map()` chains (e.g. `nodes.filter(n => n.type === 'module').map(...)` followed by the same for 'class' and 'function'). This results in multiple O(N) traversals and creates many intermediate array allocations, significantly dragging down performance for extremely large graphs.
**Action:** When categorizing or mapping large arrays into multiple subsets, avoid multiple chained `.filter().map()` operations. Instead, define empty target lists and execute a single `for...of` loop to conditionally evaluate and push items directly to their respective lists.
