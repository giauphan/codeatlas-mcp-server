...
## 2026-09-10 - Arbitrary Command Execution via cwd in shell commands
**Vulnerability:** Command injection was possible because `execSync` implicitly executed commands in a shell, allowing arbitrary command execution if the working directory (`cwd`) was somehow attacker-controlled.
**Learning:** Even local scripts executed by the node binary can be vulnerable to command injection if run through a shell (e.g., via `execSync`). To execute scripts securely, always use `spawnSync` with `shell: false`. Also, avoid adding overly strict regex validation to system paths like `process.cwd()` that contain backslashes on Windows, as this breaks cross-platform compatibility.
**Prevention:** Replace `execSync(node scripts/...)` with `spawnSync(process.execPath, [scripts/...], { shell: false })`. This executes the script without invoking a shell, inherently mitigating the command injection risk regardless of the `cwd`.
