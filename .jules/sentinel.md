...
## 2025-03-01 - Prevent Command Injection in CLI Setup Scripts
**Vulnerability:** The CLI setup scripts used `child_process.execSync` to run setup and validation shell commands (e.g. `node scripts/setup-hooks.js`). While the paths were internal, the use of `execSync` is inherently vulnerable to command injection if an attacker can control the current working directory path (which contains the repo), or any variable dynamically interpolated into the executed command string.
**Learning:** Functions like `execSync` invoke a shell by default, meaning dynamically evaluated strings or paths are processed through bash, leading to potential command injection vulnerabilities. Using `__dirname` or `process.cwd()` derived paths does not secure against command injection if the containing path itself includes shell metacharacters.
**Prevention:** Always use `child_process.execFileSync` or `child_process.spawnSync` with explicitly defined arrays for arguments and `{ shell: false }` to strictly execute binaries without a shell intermediary, effectively avoiding command injection vulnerabilities.
