...
## 2025-02-28 - Dynamic Path Command Injection Risk via execSync
**Vulnerability:** The `child_process.execSync` function was used to execute `node ${SETUP_SCRIPT}` where `SETUP_SCRIPT` was derived from `__dirname`. Because `execSync` defaults to running commands through a shell (`shell: true`), this could lead to command injection if the repository is cloned into a path containing shell metacharacters (like `&` or `;`).
**Learning:** Even when executing trusted scripts using internal paths derived from `__dirname` or `process.cwd()`, using `execSync` with interpolation introduces a vector for path-based command injection because the user controls the clone directory name. We must avoid applying "security theater" fixes to entirely static strings (e.g. `execSync("node scripts/setup.js")`), but must protect any command containing variable interpolation.
**Prevention:** Use `child_process.execFileSync` or `child_process.spawnSync` with explicitly defined arrays for arguments and `shell: false` to prevent command injection when executing commands that include dynamically resolved paths or variables.
