...
## 2025-03-01 - Prevent Command Injection via execSync with execFileSync input
**Vulnerability:** The `validate-hooks.js` script used `child_process.execSync` with string interpolation and piped commands (e.g., `echo 'data' | bash script.sh`). This pattern is vulnerable to command injection if variables contain shell metacharacters.
**Learning:** When migrating from `execSync` to `execFileSync` (with `shell: false`) to eliminate shell invocation, shell pipeline features like `|` are no longer available. However, standard input can still be securely piped to the subprocess using the `input` option of `execFileSync`.
**Prevention:** Replace shell pipelines (like `echo data | command`) with `child_process.execFileSync('command', [args], { input: 'data', shell: false })` to securely pass data to a subprocess without risking command injection.
