...
## 2024-10-25 - [Fix TOCTOU Symlink Append Vulnerability in Gitignore]
**Vulnerability:** The `mcpServer` tools used `fs.appendFileSync` and `fs.writeFileSync` to write to configuration files (e.g., `.gitignore`, `.env`, `hermesCfg`). These functions natively follow symlinks, creating a TOCTOU (Time-of-Check to Time-of-Use) vulnerability. If an attacker replaced a target file with a symlink between the existence check and the write operation, the application would follow the symlink and write to an arbitrary file.
**Learning:** `fs.appendFileSync` and `fs.writeFileSync` do not accept flags to prevent symlink following, rendering them unsafe for writing to files in directories that might be controlled or influenced by users.
**Prevention:** To prevent TOCTOU and symlink following vulnerabilities when writing or appending files, strictly avoid `fs.writeFileSync` and `fs.appendFileSync`. Obtain a secure file descriptor using `fs.openSync` with `fs.constants.O_NOFOLLOW` and the appropriate read/write/append flags, write to the descriptor, and ensure it is safely closed within a `try...finally` block. Encapsulate these operations in consolidated utility functions like `writeFileSyncNoFollow` and `appendFileSyncNoFollow`.
