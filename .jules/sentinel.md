...
## 2025-03-01 - [Fix TOCTOU Symlink Write Vulnerability in Config setup tools]
**Vulnerability:** Similar to `export_team_artifact`, tools like `setup_second_brain` and `sync_skills_inventory` were using `fs.writeFileSync` to save configurations. This creates a TOCTOU (Time-of-Check to Time-of-Use) vulnerability if the target path is swapped with a symlink.
**Learning:** Consistently using secure file writing mechanisms (`writeFileSyncNoFollow`) is necessary for any tools that generate or update file-system configurations in predictable locations (e.g. `~/.codeatlas/`, `~/.claude/`), not just export tools.
**Prevention:** Always use a safe wrapper like `writeFileSyncNoFollow` leveraging `fs.openSync` with `O_NOFOLLOW` flag when overwriting configuration files to avoid symlink traversal attacks.
