...
## 2026-09-12 - [Indirect Command Injection via Unsanitized node string in execSync]
...
## 2024-10-26 - [Fix TOCTOU Symlink Append Vulnerability in Gitignore Update]
**Vulnerability:** The `export_team_artifact` tool used `fs.appendFileSync` to update the `.gitignore` file. Since `fs.appendFileSync` intrinsically follows symlinks, this left the application vulnerable to symlink-based TOCTOU (Time-of-Check to Time-of-Use) attacks where an attacker could append arbitrary data to system files by pointing `.gitignore` to a sensitive file via a symlink.
**Learning:** Like `fs.writeFileSync`, `fs.appendFileSync` follows symlinks natively and cannot be configured to ignore them via string flags. It must be replaced with file descriptor-based logic using `O_NOFOLLOW`.
**Prevention:** To prevent TOCTOU and symlink following vulnerabilities when appending to files, avoid `fs.appendFileSync`. Instead, create a helper function (e.g. `appendFileSyncNoFollow`) that obtains a secure file descriptor using `fs.openSync` with the explicit `fs.constants.O_NOFOLLOW` flag alongside `O_CREAT | O_WRONLY | O_APPEND`, writes to the descriptor, and ensures it is reliably closed.
