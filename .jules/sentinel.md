...
## 2025-03-01 - [Fix TOCTOU Symlink Write Vulnerability in Configuration Appends]
**Vulnerability:** The application used `fs.appendFileSync` and `fs.writeFileSync(..., { flag: "a" })` to append configurations to files like `.gitignore` and `config.yaml`. These methods intrinsically follow symlinks, leaving the application vulnerable to symlink-based TOCTOU (Time-of-Check to Time-of-Use) attacks where an attacker could append arbitrary data to system files by pre-creating symlinks.
**Learning:** Functions that append to files inherently follow symlinks if not explicitly configured with `O_NOFOLLOW`.
**Prevention:** To prevent TOCTOU and symlink following vulnerabilities when appending to files, avoid `fs.appendFileSync` and `fs.writeFileSync(..., { flag: "a" })`. Instead, obtain a secure file descriptor using `fs.openSync` with the explicit `fs.constants.O_NOFOLLOW` flag alongside `O_CREAT | O_WRONLY | O_APPEND`, write to the descriptor, and ensure it is reliably closed.
