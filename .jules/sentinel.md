...
## 2024-05-18 - [Security] Consolidate Secure File Writing Utilities
**Vulnerability:** TOCTOU and symlink following vulnerabilities occurred when writing or appending sensitive configuration files using native `fs.writeFileSync` and `fs.appendFileSync`.
**Learning:** Security-critical file I/O operations must be encapsulated in consolidated utilities (e.g., `writeFileSyncNoFollow`, `appendFileSyncNoFollow`) rather than duplicated manually across tool implementations to prevent accidental bypasses and regressions. Manual usage of `fs.writeFileSync` often leads to inconsistent application of security flags.
**Prevention:** Always use the consolidated safe file utilities from `src/utils/pathUtils.ts` when performing file writes, especially for configuration and settings files in user directories.
