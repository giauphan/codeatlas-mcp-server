...
## 2024-05-24 - [Command Injection via execSync in Setup Scripts]
**Vulnerability:** Found multiple instances where Node.js internal setup and validation scripts were executed using `execSync(..., { stdio: "inherit" })`. While the arguments were static, `execSync` natively runs commands inside a shell wrapper. Additionally, one instance relied on the system's `node` binary rather than the currently executing Node.js binary, potentially allowing path hijacking or unexpected binary execution if the environment is compromised.
**Learning:** Even when the command string itself is static, executing Node.js scripts via `execSync` is inherently riskier than using `spawnSync` due to the intermediary shell and reliance on the `$PATH`. It is also less resilient for cross-platform execution compared to using `process.execPath`.
**Prevention:** Always use `spawnSync(process.execPath, ['path/to/script.js'], { shell: false })` when executing internal Node.js scripts. This avoids the shell entirely, guarantees the correct Node.js binary is used, and natively mitigates shell injection. Remember to manually check `result.error` and `result.status !== 0` to preserve the exception-throwing behavior of `execSync`.
