...
## 2026-03-12 - [Fix TOCTOU Symlink Write Vulnerability in gitignore append]
**Vulnerability:** The `export_team_artifact` tool used `fs.appendFileSync` to append configuration to `.gitignore`. Similar to `fs.writeFileSync`, `fs.appendFileSync` natively follows symlinks, leaving the system vulnerable to TOCTOU attacks where an attacker could place a symlink to overwrite or append to arbitrary sensitive files on the filesystem.
**Learning:** All Node.js `fs.*FileSync` and `fs.promises.*File` methods inherently follow symlinks. Safe appending requires obtaining a secure file descriptor via `fs.openSync` using `fs.constants.O_NOFOLLOW` flag combined with `O_APPEND`.
**Prevention:** To prevent TOCTOU and symlink following vulnerabilities when appending to files, avoid using `fs.appendFileSync`. Instead, use a centralized secure utility like `appendFileSyncNoFollow` which obtains a secure file descriptor with `fs.constants.O_NOFOLLOW` alongside `O_CREAT | O_WRONLY | O_APPEND`.
