...
## 2025-02-28 - [Fix TOCTOU Symlink Write Vulnerability in Config and Append Operations]
**Vulnerability:** Similar to the previous finding on `export_team_artifact`, several configuration updates (e.g., `~/.hermes/config.yaml`, `~/.claude/claude.json`, `.env`) and the `.gitignore` append operation in `mcpServer.ts` were utilizing standard `fs.writeFileSync` and `fs.appendFileSync` methods. Since these operations operate on paths located in the user's home directory or workspace, they are susceptible to Time-of-Check to Time-of-Use (TOCTOU) symlink attacks if the directories are untrusted or shared.
**Learning:** `fs.appendFileSync` and `fs.writeFileSync` both resolve and follow symlinks implicitly. Relying on these functions for operations in user or workspace directories can lead to arbitrary file overwrites or appends if an attacker pre-creates symlinks at the target paths.
**Prevention:** Extend the TOCTOU prevention strategy to all configuration and append operations. Always use `fs.openSync` with `fs.constants.O_NOFOLLOW` (and `O_CREAT | O_WRONLY` with either `O_TRUNC` for overwrite or `O_APPEND` for append) to obtain a secure file descriptor before writing data.
