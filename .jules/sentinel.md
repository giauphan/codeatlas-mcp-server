...
## 2025-02-28 - [Fix TOCTOU Symlink Write/Append Vulnerability in Artifact Export]
**Vulnerability:** The `export_team_artifact` tool used `fs.writeFileSync` and `fs.appendFileSync` to write and append to files like `artifact-summary.json`, `artifact.json`, and `.gitignore`. Both `writeFileSync` and `appendFileSync` natively follow symlinks, leaving the application vulnerable to symlink-based TOCTOU (Time-of-Check to Time-of-Use) attacks where an attacker could overwrite or append arbitrary content to critical system files by pre-creating symlinks.
**Learning:** File write and append operations must enforce symlink protection explicitly if the path is dynamically constructed or influenced by user/external state. The standard Node.js APIs (`fs.appendFileSync`, `fs.writeFileSync`) default to following symlinks, and string flags like `"w"` or `"a"` are insufficient to prevent this.
**Prevention:** To prevent TOCTOU and symlink following vulnerabilities, avoid direct use of `fs.writeFileSync` and `fs.appendFileSync`. Instead, obtain a secure file descriptor using `fs.openSync` with the explicit `fs.constants.O_NOFOLLOW` flag alongside `O_CREAT | O_WRONLY | O_TRUNC` (for writes) or `O_CREAT | O_WRONLY | O_APPEND` (for appends). Manage the file descriptor lifecycle safely using a `try...finally` block. Consolidated utilities (`writeFileSyncNoFollow`, `appendFileSyncNoFollow`) should be used to prevent such vulnerabilities across the codebase.
## 2025-03-08 - [Enforce safe file writes across codebase]
**Vulnerability:** Scattered usage of `fs.writeFileSync` and `fs.appendFileSync` natively follow symlinks, leading to TOCTOU vulnerabilities.
**Learning:** Even internal configuration writes or client configuration updates must be secured against symlink attacks.
**Prevention:** Replace all `fs.writeFileSync` with `writeFileSyncNoFollow` and `fs.appendFileSync` with `appendFileSyncNoFollow` from `src/utils/pathUtils.ts`.
