...
## 2025-02-28 - [Fix TOCTOU Symlink Append Vulnerability]
**Vulnerability:** The `export_team_artifact` tool used `fs.appendFileSync` to append to the project's `.gitignore` file. `fs.appendFileSync` natively follows symlinks, exposing the system to TOCTOU attacks where a malicious `.gitignore` symlink could allow appending arbitrary content to critical system files.
**Learning:** File append operations, similar to file write operations, must enforce symlink protection explicitly if the path is dynamically constructed or influenced by user/external state. The standard Node.js APIs (`fs.appendFileSync`, `fs.writeFileSync`) default to following symlinks.
**Prevention:** Always use `fs.openSync` (or its async equivalent) with the `fs.constants.O_NOFOLLOW` flag alongside `O_CREAT | O_WRONLY | O_APPEND` when appending to files, and manage the file descriptor lifecycle safely. A consolidated utility (`appendFileSyncNoFollow`) should be used to prevent such vulnerabilities across the codebase.
