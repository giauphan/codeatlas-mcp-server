...
**Vulnerability:** Command Injection in `src/cli/setup-claude.js` via `execSync` which implicitly invokes a shell (e.g. `/bin/sh -c`). If the derived `SETUP_SCRIPT` path happens to contain shell metacharacters (e.g. spaces, semicolons, backticks) because the repository was cloned into an unusually named or attacker-controlled directory, the shell could execute arbitrary commands instead of properly routing the argument to Node.js.
**Learning:** For dynamic script execution, `spawnSync` is the secure alternative to `execSync` as it defaults to `shell: false`. This inherently prevents command injection by bypassing the shell entirely and passing arguments directly to the target executable. However, developers must remember that `spawnSync` does not automatically throw an exception when the child process exits with a non-zero status code like `execSync` does, which could lead to silent failures.
**Prevention:** Always prefer `spawnSync` with array arguments over `execSync` for dynamic script execution. When migrating from `execSync`, ensure you manually replicate the original error propagation behavior by explicitly checking and throwing on `result.error` and `result.status !== 0`.
