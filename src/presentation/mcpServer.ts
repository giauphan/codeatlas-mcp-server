        ...
        try {
          if (fs.existsSync(hermesCfg)) {
            ...
            if (cfg.includes("codeatlas:")) {
              ...
              // writeFileSyncNoFollow does not support { flag: "a" }, so we must read, append and write back,
              // but since we checked above if it exists, and it's already in 'cfg', we append to 'cfg'
              writeFileSyncNoFollow(hermesCfg, cfg + "\nmcp_servers:\n" + mcpEntry);
