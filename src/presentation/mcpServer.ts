...
(Data kept in memory and background sync to CodeAtlas Cloud initiated)`;
  ...
  server.tool(
    ...
    async ({ project, commits }: { project?: string; commits?: number }) => {
      ...
      try {
        result.branch = cp.execFileSync("git", ["rev-parse", "--abbrev-ref", "HEAD"], { cwd: resolvedDir, encoding: "utf-8", shell: false }).toString().trim();
        const st = cp.execFileSync("git", ["status", "--porcelain"], { cwd: resolvedDir, encoding: "utf-8", shell: false }).toString();
        ...
        try {
          const [behind, ahead] = cp.execFileSync("git", ["rev-list", "--left-right", "--count", "HEAD...@{upstream}"], { cwd: resolvedDir, encoding: "utf-8", shell: false }).toString().trim().split("\t").map(Number);
          result.ahead = ahead || 0; result.behind = behind || 0;
        } catch { result.ahead = null; result.behind = null; }
        const logRaw = cp.execFileSync("git", ["log", `-${maxC}`, "--format=COMMIT%n%H%n%an%n%ai%n%s%nFILES:", "--name-only"], { cwd: resolvedDir, encoding: "utf-8", maxBuffer: 1024 * 1024, shell: false }).toString();
