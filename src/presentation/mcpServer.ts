    ...
    async ({ action, query, limit }) => {
      ...
      if (action === "query") {
        ...
        if (qRaw) {
          ...
          const regex = new RegExp(qRaw.replace(/[.*+?^${}()|[\]\\]/g, '\\$&'), 'i');
          for (const s of skills) {
            if (s.name.includes(qLower) || regex.test(s.description)) {
              matchCount++;
        ...
        return { content: [{ type: "text" as const, text: JSON.stringify({
          query: qRaw ? qRaw.toLowerCase() : "(all)", count: matchCount, totalSkills: skills.length,
          results,
        }, null, 2) }] };
      }
