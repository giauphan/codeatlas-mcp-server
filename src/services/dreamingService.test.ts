...
function mockSuccessResponse(
  statusCode: number,
  body: unknown,
  capture?: { options?: any; written?: string },
) {
  ...
  return (options: any, callback: (res: any) => void) => {
    ...
    const mockReq: any = {
      ...
      on(event: string, handler: (...args: any[]) => void) {
        if (event === "error") mockReq._errorHandler = handler;
        return mockReq;
      },
      write(data: any) {
        if (capture) capture.written = String(data);
      },
      end() {
        callback(mockRes);
      },
...
describe("Dreaming Service — HTTPS Client", () => {
    ...
    mock.module("https", {
      namedExports: {
        request(options: any, callback: (res: any) => void) {
          if (!requestMock)
            throw new Error(
              "requestMock is not set — did you forget to assign it in a test?",
            );
  ...
  describe("saveDreamMemory()", () => {
    ...
    it("sends apiKey in both query param and x-api-key header", async () => {
      ...
      assert.ok(
        String(opts.path).includes("apiKey=test-key-sentinel"),
        `path should contain apiKey query param, got: ${opts.path}`,
      );
  ...
  describe("queryDreamMemories()", () => {
    ...
    it("throws when API response is not valid JSON", async () => {
      ...
      await assert.rejects(
        () => dreamingService.queryDreamMemories({ query: "x" }),
        { message: /Failed to parse dream memory query response/ },
      );
    });
    ...
    it("throws when API returns non-2xx status", async () => {
      ...
      await assert.rejects(
        () => dreamingService.queryDreamMemories({ query: "x" }),
        { message: /Failed to query dream memories: status 403/ },
      );
    });
    ...
    it("sends `limit` as a query parameter when provided", async () => {
      ...
      assert.ok(
        path.includes("limit=5"),
        `Expected limit=5 in path, got: ${path}`,
      );
    });
    ...
    it("does NOT send `limit` when omitted", async () => {
      ...
      assert.ok(
        !path.includes("limit="),
        `Expected no limit param, got: ${path}`,
      );
    });
    ...
    it("sends the `project` filter when provided", async () => {
      ...
      const path = String(capture.options!.path);
      assert.ok(
        path.includes("project=my-app"),
        `Expected project=my-app in path, got: ${path}`,
      );
    });
    ...
    it("includes apiKey in query params and x-api-key header for GET", async () => {
      ...
      const path = String(capture.options!.path);
      assert.ok(
        path.includes("apiKey=test-key-query"),
        `Expected apiKey in query, got: ${path}`,
      );
      assert.strictEqual(
        capture.options!.headers["x-api-key"],
        "test-key-query",
      );
    });
  });
