...
describe("Dreaming Service - HTTPS Client", () => {
  ...
  describe("saveDreamMemory()", () => {
    ...
    it("does NOT send apiKey in query param but sends x-api-key header", async () => {
      ...
      assert.ok(!String(opts.path).includes("apiKey=key-sentinel"), `path should not contain apiKey, got: ${opts.path}`);
  ...
  describe("queryDreamMemories()", () => {
    ...
    it("does not include trailing ? when no optional params are sent", async () => {
      const capture: { options?: any } = {};
      requestMock = mockSuccessResponse(200, { memories: [] }, capture);
    ...
    it("does NOT include apiKey in query params but sends x-api-key header for GET", async () => {
      ...
      const path = String(capture.options!.path);
      assert.ok(!path.includes("apiKey=test-key-query"), `Expected no apiKey in query, got: ${path}`);
